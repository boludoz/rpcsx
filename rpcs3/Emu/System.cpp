...
static FileType getFileType(const fs::file& file, const std::string& path = {})
{
	file.seek(0);
...
game_boot_result Emulator::BootGame(std::string path, const std::string& title_id, bool direct, cfg_mode config_mode, const std::string& config_path)
{
	...
	{
		fs::file file(path);
		if (getFileType(file, path) == FileType::Iso)
		{
			load_iso(path);

			auto mount_path = iso_device::virtual_device_name + "/";
